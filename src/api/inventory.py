...
def get_inventory():
    ...
    num_potions_w = "SELECT num_white_potions FROM global_inventory"
    ...
    num_ml_w = "SELECT num_white_ml FROM global_inventory"
    ...
    with db.engine.begin() as connection:
        ...
        num_p_w = connection.execute(sqlalchemy.text(num_potions_w)).scalar()
        ...
        num_mili_w = connection.execute(sqlalchemy.text(num_ml_w)).scalar()
...
def get_capacity_plan():
    ...
    num_potions_w = "SELECT num_white_potions FROM global_inventory"
    num_ml_g = "SELECT num_green_ml FROM global_inventory"
    num_ml_r = "SELECT num_red_ml FROM global_inventory"
    num_ml_b = "SELECT num_blue_ml FROM global_inventory"
    num_ml_w = "SELECT num_white_ml FROM global_inventory"
    gold_sql = "SELECT gold FROM global_inventory"
    ...
    with db.engine.begin() as connection:
        ...
        num_mil_w = connection.execute(sqlalchemy.text(num_ml_w)).scalar()
        gold = connection.execute(sqlalchemy.text(gold_sql)).scalar()

        total_p = num_p_g + num_p_r + num_p_b + num_p_w
        total_ml = num_mil_g + num_mil_r + num_mil_b + num_mil_w
    ...
    if gold >= 1000:
        ...
        with db.engine.begin() as connection:
            inventory_update = """UPDATE global_inventory SET
                                    num_red_potions = num_red_potions + (:potion * 50),
                                    num_green_potions = num_green_potions + (:potion * 50),
                                    num_blue_potions = num_blue_potions + (:potion * 50),
                                    num_white_potions = num_white_potions + (:potion * 50),
                                    num_red_ml = num_red_ml + (:ml * 10000),
                                    num_green_ml = num_green_ml + (:ml * 10000),
                                    num_blue_ml = num_blue_ml + (:ml * 10000),
                                    num_white_ml = num_white_ml + (:ml * 10000)"""
            gold_update = "UPDATE global_inventory SET gold = gold - (1000 * :ml_cap * :p_cap)"
            new_inventory = connection.execute(sqlalchemy.text(inventory_update), {"potion": int(potion_cap / 4), "ml": int(ml_cap / 4)})
